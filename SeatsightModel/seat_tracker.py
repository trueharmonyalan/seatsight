...
class SeatTracker:
    ...
    def set_seat_config(self, seats):
        ...
        Args:
            seats: List of seat dictionaries with id, seat_number, position, etc.
                   Expected key 'position' is a tuple (x, y) representing the expected seat center.
        """
        # Update seat limit based on config
        self.seat_limit = len(seats)
        self.logger.info(f"Set configuration for {self.seat_limit} seats")
        
        # Initialize seat status using the configured seat number and expected position if provided.
        self.seat_status = []
        for seat in seats:
            expected_centroid = seat.get('position')  # Expected as a tuple, e.g., (x, y)
            self.seat_status.append({
                "seatNumber": seat['seat_number'],
                "status": "vacant",
                "bbox": None,
    ...
    def process_frame(self, frame):
        ...
        try:
            ...
            # Determine if preconfigured seats include positions; if so, map by proximity.
            mapping_done = False
            with_positions = all(seat.get("expected_centroid") is not None for seat in self.seat_status)
            new_seat_status = []
            if with_positions:
                # Make a copy of current config (vacant seats) for matching.
                unmatched_seats = {seat["id"]: seat for seat in self.seat_status}
                # For each detection, assign the closest configured seat that hasn't been updated.
                for detection in detections_list:
                    best_seat = None
                    best_distance = None
                    for seat in unmatched_seats.values():
                        expected = seat["expected_centroid"]
                        distance = math.hypot(detection["centroid"][0] - expected[0],
                                              detection["centroid"][1] - expected[1])
                        if best_distance is None or distance < best_distance:
                            best_distance = distance
                            best_seat = seat
                    # Update the seat if found
                    if best_seat:
                        updated_seat = best_seat.copy()
                        updated_seat.update({
                            "bbox": detection["bbox"],
                        ...
                        new_seat_status.append(updated_seat)
                        # Mark as matched by removing from unmatched
                        unmatched_seats.pop(best_seat["id"])
                
                # Preserve remaining unmatched seats as vacant using their original configuration.
                new_seat_status.extend(list(unmatched_seats.values()))
                # If for any reason there are more seats than seat_limit, slice the list.
                new_seat_status = sorted(new_seat_status, key=lambda s: s["seatNumber"])[:self.seat_limit]
                mapping_done = True
            
            if not mapping_done:
                # Fallback: sort detections by position (top-to-bottom, then left-to-right) and assign sequentially.
                detections_list.sort(key=lambda d: (d["centroid"][1], d["centroid"][0]))
                new_seat_status = []
                for idx, detection in enumerate(detections_list):
                    if idx >= self.seat_limit:
                        break
                    # Use the preconfigured id and seatNumber if available.
                    preconf = self.seat_status[idx] if idx < len(self.seat_status) else {}
                    detection.update({
                        "seatNumber": preconf.get("seatNumber", idx + 1),
                        "id": preconf.get("id", idx + 1)
                    })
                    new_seat_status.append(detection)
                
                # Pad with vacant seats if needed
                while len(new_seat_status) < self.seat_limit:
                    seat_idx = len(new_seat_status)
                    preconf = self.seat_status[seat_idx] if seat_idx < len(self.seat_status) else {}
                    new_seat_status.append({
                        "seatNumber": preconf.get("seatNumber", seat_idx + 1),
                        "id": preconf.get("id", seat_idx + 1),
                        "status": "vacant",
                        "bbox": None,
                        "centroid": None,
                        "predicted_class": "empty_seat",
                        "confidence": 0.0,
                        "expected_centroid": preconf.get("expected_centroid")
                    })
            
            # Update seat status in a thread-safe fashion.
            with self.lock:
                self.seat_status = new_seat_status
